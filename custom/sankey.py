...
from config import PATHWAY_PATH, REACTOM_PATHWAY_PATH, RESULT_PATH
...
def get_first_layer_df(nlargest):
    features_weights = pd.read_csv(join(RESULT_PATH, 'extracted/gradient_importance_0.csv'), index_col=[0, 1])
    features_weights['layer'] = 0
    nodes_per_layer0 = features_weights[['layer']]
    features_weights = features_weights[['coef']]

    all_weights = pd.read_csv(join(RESULT_PATH, 'extracted/node_importance_graph_adjusted.csv'), index_col=0)
...
def get_links_with_first_layer():
    ...
    link = pd.read_csv(join(RESULT_PATH, 'extracted/gradient_importance_0.csv'), index_col=[0, 1])
    link = link.reset_index()
    link.columns = ['target', 'source', 'value']
    link['layer'] = 0
    links.append(link)
    for l in range(1, 7):
        link = pd.read_csv(join(RESULT_PATH, 'extracted/link_weights_{}.csv'.format(l)), index_col=0)
...
def get_x_y(df_encoded, layers_nodes):
    ...
    :param layers_nodes: data_access frame with index (nodes ) and one columns (layer) representing the layer of the node
...
def get_node_colors_ordered(high_nodes_df, col_name, remove_others=True):
    '''
    high_nodes_df: data_access frame with index (node names) andcolumns [layer, coef_combined]
    '''
    node_colors = {}
...
def run(model_name=None):
    ...
    node_importance = pd.read_csv(join(RESULT_PATH, 'extracted/node_importance_graph_adjusted.csv'), index_col=0)
    #####
