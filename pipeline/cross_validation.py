...
def run():
    ...
    X = np.concatenate((all_data.x_train, all_data.x_validate_), axis=0)
    y = np.concatenate((all_data.y_train, all_data.y_validate_), axis=0)
    ...
    for epoch in range(n_epochs):
        ...
        for train_index, test_index in skf.split(X, y.ravel()):
            ...
            model.to('cpu')
            i = i + 1
    results.to_csv(join(LOG_PATH, 'cross_valida_result.csv'))
